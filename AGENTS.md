...
Codex Proxy is an HTTP proxy that translates Codex API requests to OpenAI-compatible format, enabling Codex to work with 200+ alternative models through OpenRouter, OpenAI Direct (o1/o3), and Ollama (local). The proxy runs as a daemon, performs bidirectional API format conversion, and maintains full Codex feature compatibility including tool calling, extended thinking blocks, and streaming.
...
go build -o Codex-proxy ./cmd/Codex-proxy && ./Codex-proxy -s
```
...
1. **Codex** → sends Codex API format request to `localhost:8082`
2. **handlers.go** → receives `/v1/messages` POST request
3. **converter.go** → transforms Codex format → OpenAI format
   ...
   - Maps Codex model name to target provider model using pattern-based routing
...
6. **converter.go** → transforms OpenAI format → Codex format
7. **handlers.go** → returns Codex-format response to Codex
...
- Extracts `reasoning_details` array → converts to Codex `thinking` blocks
...
- Codex `tool_use` content blocks → OpenAI `tool_calls` array
- OpenAI `tool_calls` → Codex `tool_use` blocks
...
- OpenRouter `reasoning_details` → Codex `thinking` block with `signature` field
- `signature` field is REQUIRED for Codex to hide/show thinking properly
- Without signature, thinking appears as regular text in chat

**Streaming** (`streamOpenAIToClaude` in handlers.go):
- Converts OpenAI SSE chunks (`data: {...}`) → Codex SSE events
...
2. `~/.Codex/proxy.env` (recommended location)
3. `~/.Codex-proxy` (legacy location)
...
- Test Codex → OpenAI message conversion
...
To manually test the proxy with Codex CLI:
...
### 2. Test with different Codex model tiers

The proxy routes Codex model names to your configured backend models:

```bash
# Test with Opus tier (routes to ANTHROPIC_DEFAULT_OPUS_MODEL or gpt-5)
ANTHROPIC_BASE_URL=http://localhost:8082 Codex --model opus -p "hi"

# Test with Sonnet tier (routes to ANTHROPIC_DEFAULT_SONNET_MODEL or gpt-5)
ANTHROPIC_BASE_URL=http://localhost:8082 Codex --model sonnet -p "hi"

# Test with Haiku tier (routes to ANTHROPIC_DEFAULT_HAIKU_MODEL or gpt-5-mini)
ANTHROPIC_BASE_URL=http://localhost:8082 Codex --model haiku -p "hi"
```
...
ANTHROPIC_BASE_URL=http://localhost:8082 Codex --model sonnet -p "list files in current directory"

# Should see tool_calls in debug logs
# Verify proper Codex tool_use → OpenAI tool_calls → Codex tool_result conversion
```
...
ANTHROPIC_BASE_URL=http://localhost:8082 Codex --model sonnet -p "solve: 2x + 5 = 15"

# Should show thinking process in Codex UI
...
ANTHROPIC_BASE_URL=http://localhost:8082 Codex --model opus -p "think through this problem"
```
...
2. **Thinking blocks need signature field**: Without `signature: "..."` field, Codex shows thinking as plain text instead of hiding it
...
- Creates PID file at `/tmp/Codex-proxy.pid`
- Redirects stdout/stderr to `/tmp/Codex-proxy.log`
- `./Codex-proxy status` checks if process is running
- `./Codex-proxy stop` kills the daemon via PID file

When testing locally, use `-d` flag for debug logging to see full requests/responses.

## Package Structure

- `cmd/Codex-proxy/main.go` - Entry point, CLI arg parsing
- `internal/config/` - Environment variable loading, provider detection
- `internal/converter/` - Codex ↔ OpenAI format conversion logic
- `internal/server/` - HTTP server (Fiber), request handlers, streaming
- `internal/daemon/` - Process management, PID file handling
- `pkg/models/` - Shared type definitions for Codex and OpenAI formats
- `scripts/ccp` - Wrapper script that starts daemon and execs Codex

## Key Files

- `internal/converter/converter.go:ConvertRequest()` - Codex → OpenAI request conversion with provider-specific parameters
- `internal/converter/converter.go:ConvertResponse()` - OpenAI → Codex response conversion, thinking block extraction
