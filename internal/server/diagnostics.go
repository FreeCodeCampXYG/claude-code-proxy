...
const diagnosticsCaptureInputLimit = diagnosticsBodyLimit * 2
...
func (trace *diagnosticsTrace) capture(boundary diagnostics.ContentBoundary, attempt int, body []byte) {
	...
	captureBody := body
	captureMode := diagnostics.ContentCaptureFull
	if len(body) > diagnosticsCaptureInputLimit {
		captureMode = diagnostics.ContentCaptureSummary
		sum := sha256.Sum256(body)
		captureBody, _ = json.Marshal(map[string]interface{}{
			"partial":          true,
			"reason":           "capture_input_limit",
			"length":           len(body),
	...
	trace.mu.Lock()
	defer trace.mu.Unlock()
	trace.captures = append(trace.captures, diagnostics.ContentCapture{
		RequestID:     trace.event.RequestID,
		AttemptNumber: attempt,
		Boundary:      boundary,
		CreatedAt:     time.Now().UTC(),
		CaptureMode:   captureMode,
		Body:          append([]byte(nil), captureBody...),
	})
}
