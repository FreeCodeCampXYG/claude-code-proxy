...
func TestOpenRejectsIncompatibleDiagnosticsContentForeignKey(t *testing.T) {
	...
	// Start from the canonical events schema so Open reaches the content-FK check.
	store, err := Open(path, StoreOptions{CaptureContent: true})
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = db.Exec(`DROP TABLE diagnostics_content;
		CREATE TABLE diagnostics_content (
			...
			FOREIGN KEY(request_id) REFERENCES diagnostics_events(request_id)
		);`)
	if closeErr := db.Close(); closeErr != nil {
		t.Fatal(closeErr)
	}
